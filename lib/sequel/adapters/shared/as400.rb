...
module Sequel
  module AS400
    Sequel::Database.set_shared_adapter_scheme(:as400, self)
    ...
    module DatabaseMethods
      ...
      def as400_version
        return @as400_version if defined?(@as400_version)
        @as400_version = metadata_dataset.with_sql("select os_version || '.' || os_release as os_level from sysibmadm.env_sys_info").first[:os_level]
      end
      alias_method :server_version, :as400_version
      ...
      # Use INFORMATION_SCHEMA.COLUMNS to get the information on the tables.
      def schema_parse_table(table, opts = OPTS)
        m = output_identifier_meth(opts[:dataset])
        im = input_identifier_meth(opts[:dataset])
        metadata_dataset.with_sql("SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = #{literal(im.call(table))} ORDER BY ORDINAL_POSITION").
          collect do |column| 
            column[:db_type] = column.delete(:data_type)
            if column[:db_type] =~ /\A(VAR)?CHAR\z/
              column[:db_type] << "(#{column[:character_maximum_length]})"
            end
            if column[:db_type] == "DECIMAL"
              column[:db_type] << "(#{column[:numeric_precision]},#{column[:numeric_scale]})"
            end
            column[:allow_null] = column.delete(:is_nullable) == 'YES'
            identity = column.delete(:is_identity) =='YES'
            # if column[:primary_key] = identity || !column[:keyseq].nil?
            if column[:primary_key] = identity
              column[:auto_increment] = (not column[:identity_generation].nil?)
            end
            column[:type]        = schema_column_type(column[:db_type])
            column[:max_length]  = column[:character_maximum_length] if column[:type] == :string
            [ m.call(column.delete(:name)), column]
          end
      end

      # Use QSYS2.SYSTABLES to get the tables for the database
      def tables
        metadata_dataset.
          with_sql("SELECT TABLE_NAME FROM QSYS2.SYSTABLES WHERE TABLE_TYPE IN ('P', 'V') AND TABLE_SCHEMA = #{literal(input_identifier_meth.call(opts[:schema]))}").
          all.map{|h| output_identifier_meth.call(h[:table_name]) }
      end

      # Use INFORMATION_SCHEMA.TABLE to get the views for the database
      def views
        metadata_dataset.
          with_sql("SELECT TABLE_NAME FROM QSYS2.SYSTABLES WHERE TABLE_TYPE='V' AND TABLE_SCHEMA = #{literal(input_identifier_meth.call(opts[:schema]))}").
          all.map{|h| output_identifier_meth.call(h[:table_name]) }
      end

      # Use QSYS2.SYSPARTITIONINDEXES to get the indexes for the table
      def indexes(table, opts = OPTS)
        m = output_identifier_meth
        table = table.value if table.is_a?(Sequel::SQL::Identifier)
        indexes = {}
        metadata_dataset.
         from(Sequel[:qsys2][:syspartitionindexes]).
         select(:index_name, :unique, :column_names).
         where(:table_name=>input_identifier_meth.call(table), :system_required=>0).
         each do |r|
          indexes[m.call(r[:index_name])] = {:unique=>(r[:is_unique]=='UNIQUE'), :columns=>r[:column_names][1..-1].split('+').map{|v| m.call(v)}}
        end
        indexes
      end
      ...
      # AS400 requires parens around the SELECT, and WITHOUT DATA at the end.
      def create_table_as_sql(name, sql, options)
        "#{create_table_prefix_sql(name, options)} AS (#{sql}) WITHOUT DATA"
      end

      # Here we use DGTT which has most backward compatibility, which uses
      # DECLARE instead of CREATE. Creates a table in QTEMP, which is JOB-local!
      ...
      # AS400 has issues with quoted identifiers, so
      # turn off database quoting by default.
      def quote_identifiers_default
        false
      end

      # AS400 uses RENAME TABLE to rename tables.
      ...
      # AS400 uses smallint to store booleans.
      def type_literal_generic_trueclass(column)
        :smallint
      end
      alias type_literal_generic_falseclass type_literal_generic_trueclass

      # AS400 uses clob for text types.
      def uses_clob_for_text?
        true
      end

      # AS400 supports views with check option.
    ...
    module DatasetMethods
      ...
      # AS400 supports GROUP BY CUBE
      def supports_group_cube?
        true
      end

      # AS400 supports GROUP BY ROLLUP
      def supports_group_rollup?
        true
      end

      # AS400 supports GROUPING SETS
      def supports_grouping_sets?
        true
      end

      # AS400 does not support IS TRUE.
      def supports_is_true?
        false
      end

      # AS400 supports lateral subqueries
      def supports_lateral_subqueries?
        true
      end
      
      # AS400 supports MERGE
      def supports_merge?
        true
      end

      # AS400 does not support multiple columns in IN.
      def supports_multiple_column_in?
        false
      end

      # AS400 only allows * in SELECT if it is the only thing being selected.
      def supports_select_all_and_column?
        false
      end

      # AS400 supports window functions
      def supports_window_functions?
        true
      end

      # AS400 does not support WHERE 1.
      ...
      # strategy is used without a limit, as AS400 doesn't support that syntax with
      # no limit.
      def emulate_offset_with_row_number?
        super && (db.offset_strategy == :emulate || (db.offset_strategy == :limit_offset && !@opts[:limit]))
      end

      # AS400 needs the standard workaround to insert all default values into
      # a table with more than one column.
      def insert_supports_empty_values?
        false
      end

      # Use 0 for false on AS400
      def literal_false
        '0'
      end

      # AS400 doesn't support fractional seconds in times, only fractional seconds in timestamps.
      def literal_sqltime(v)
        v.strftime("'%H:%M:%S'")
      end

      # Use 1 for true on AS400
      def literal_true
        '1'
      end

      # AS400 uses a literal hexadecimal number for blob strings
      ...
      # AS400 can insert multiple rows using a UNION
      ...
      # AS400 does not require that ROW_NUMBER be ordered.
      def require_offset_order?
        false
      end

      # At least some versions of AS400 do not support NULLS FIRST/LAST.
      ...
      # AS400 supports quoted function names.
      def supports_quoted_function_names?
        true
      end

      def _truncate_sql(table)
        # "TRUNCATE #{table} IMMEDIATE" is only for newer version of as400, so we
